...
class SaveReplayCallback(DefaultCallbacks):
    ...
    def __init__(
            self,
            replay_config={},
            legacy_callbacks_dict: Dict[str, Any] = None
            ):
        """
        Args:
            replay_config -- a config dictionary with optional keys:
                ...
                'replay_folder': pathlike to base folder of replay storage
                    sub folders for train and/or eval will be made based on
                    'eval_mode'. trial folders will be created with timestamp
                    default -- './replays'
                'eval_mode': Bool whether .evaluation() trial type or not, for
                    folder naming. default -- False
                'file_type': '.mp4' or '.gif'. '.gif' is not recommended due
                    to file sizes default -- '.mp4'
        """
        ...
        assert self.file_type in ['.gif', '.mp4'], \
            'Replay file_type must be .mp4 or .gif'
        self.eval_mode = replay_config.get('eval_mode', False)
        # init best reward for recording only best episodes
        self.best_metric_max = -np.infty
        if self.eval_mode:
        ...
        trial_list = sorted(
            glob.glob(os.path.join(self.base_folder, 'run_*')),
            key=lambda x: x.split('_')[-1]
        )
        if trial_list:
            time = datetime.strptime(
                trial_list[-1].split('_')[-1],
    ...
    def on_episode_step(
        ...
        if self.record_mode == 'off':
            return  # skip all this if not recording
    ...
    def on_episode_end(
            ...
            *,
            ...
            env_index: int | None = None, **kwargs) -> None:
        if self.record_mode == 'off':
            return  # skip all this if not recording

        # get reward for this episode
        total_metric = round(episode.total_reward, ndigits=2)
        saved_replay_list = glob.glob(
            os.path.join(self.results_folder, '*'+self.file_type))
        saved_reward_list = [
            float(os.path.split(file)[-1].split('_')[0])
            for file in saved_replay_list
        ]
        if saved_replay_list:
            self.best_metric_max = max(
                self.best_metric_max, sorted(saved_reward_list)[-1])

        # Store this episode as the new best for this trial
        if (self.best_metric_max < total_metric) and \
           (len(episode.user_data['img_list']) > 0):
            ...
            now = datetime.now(tzinfo)
            now_str = now.strftime('%Y%m%d-%H%M%S')
            filename = f"{self.best_metric_max:.2f}_reward_{now_str}_" \
                       f"{id_generator()}"
    ...
    def make_mp4(self, filepath, img_list):
        ...
        imageio.mimsave(filepath+'.mp4', img_list)
...
class FlapActionMetricCallback(DefaultCallbacks):
    """
    Keeps track of actions taken, saves a ratio (percentage-like) of actions
    that were flaps as a custom metric 'flap_ratio'
    """
    def __init__(
            self,
            report_hist=False,
    ...
    def on_episode_end(
        ...
        if action_list:
            ...
            print('WARNING: There were no actions in the episode.user_data '
                  'list!')
