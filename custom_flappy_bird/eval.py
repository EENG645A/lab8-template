...
This script takes whatever checkpoint path you put on the variable
'checkpoint', then loads it and runs it for 'eval_duration' number of episodes

This is useful to evaluate how your model actually does post training
according to your design of experiments.
Assuming the performance is whatever the end of a mean metric graph is does
not tell the full story, because RL policies can often
be inconsistent or fail in some important edge cases while performing well in
others.
...
from utils.rllib_callbacks import (
    CustomScoreCallback,
...
# Or in vscode you may right click on a folder and choose "copy path"
# to get the absolute path
checkpoint = (
    '/root/lab8-template/ray_results/Lab8FlappyBirdFIXME/'
    'PPO_flappybird_5fd0b_00000_0_2024-02-21_21-15-44/checkpoint_000000'
)
replay_folder = (
...
def evaluate(
    ...
    replay_folder: str | os.PathLike,
    evaluation_duration: int = 20,
    evaluation_num_workers: int = 5
):
    """
    Runs a model evaluation from checkpoint for a duration with num workers
    and saves replays to folder
    """

    # register_env("flappybird",
    ...
    config_update = {
        ...
        # # NOTE: DO NOT turn explore off with policy algs like PPO
        # 'explore': False,
    }
    ...
    eval_results = alg.evaluate(
        duration_fn=eval_duration_fn_args(
            eval_duration=evaluation_duration, tic=tic
        )
    )
    ...
    print('Score mean: ',
          eval_results['evaluation']['custom_metrics']['score_mean'])
    seconds = time.perf_counter() - tic
    mm, ss = divmod(seconds, 60)
    hh, mm = divmod(mm, 60)
